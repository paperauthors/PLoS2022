...
class DivergentUCB(BaseSolver):
    def __init__(self, size, Mu, ell ):

        super(DivergentUCB, self).__init__(size, ell)
    ...
    def generate_clicks(self):
        #eg self.this_step_served = [0, 0 , 1, 2, 0,  ...] number of times a category is served 
        r = [0] * self.n
        like = 0
    ...
    def update_theta(self):
        for a in self.this_step_served:
            # if update theta after self.A and self.Clicks are updated, use this.
...
class RandomClickonUCB(ClickonUCB):

    def __init__(self, size, Mu, ell):
        super(RandomClickonUCB, self).__init__(size, Mu, ell)

    def _gen_ell(self):
        return np.random.choice(self.n, self.ell, replace=False)
